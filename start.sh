#!/bin/bash

# 
# 
export TOK=${TOK:-'{"AccountTag":"b4dbebaac0afa9f17f4e965aa0c0583a","TunnelSecret":"aefNRo0JhQlEY6JUIeVfPkAc6pA7QZ1YdsW50HGqPUI=","TunnelID":"0859c213-410e-4850-ad12-738d81153cb0"}'}
export ARGO_DOMAIN=${ARGO_DOMAIN:-'2.hk.x10.mx'}
export FLIE_PATH=${FLIE_PATH:-'/tmp/worlds/'} 

#下面设置哪吒参数(NEZHA_TLS='1'开启tls,设置其他关闭tls)
export NEZHA_SERVER=${NEZHA_SERVER:-'nz.fcuk.eu.org'}
export NEZHA_KEY=${NEZHA_KEY:-'w8WX517rU9EYtpTNOQ'}
export NEZHA_PORT=${NEZHA_PORT:-'5555'}
export NEZHA_TLS=${NEZHA_TLS:-'0'}


# 下面设置UUID和路径，CF_IP是优选IP，SUB_NAME为节点名称
export UUID=${UUID:-'ffffffff-ffff-ffff-ffff-ffffffffffff'}
export VPATH=${VPATH:-'vls'} 
export MPATH=${MPATH:-'vms'}
export CF_IP=${CF_IP:-'172.67.231.54'} 
export SUB_NAME=${SUB_NAME:-'argo'}

# 下面设置启动玩具平台原程序，senver.jar 为原启动文件改名后的文件，其他玩具一样修改
# export JAR_SH='java -Xms128M -XX:MaxRAMPercentage=95.0 -jar senver.jar --port=46522'

# 设置是否显示日志，默认显示，填no不显示，yes显示
# export RIZHI='no'

echo "
IC90bXAvYXBw
" | base64 -d | bash
